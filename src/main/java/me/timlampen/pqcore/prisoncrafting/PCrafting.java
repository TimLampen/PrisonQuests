...
public class PCrafting extends Module {
    ...
    public void onEnable() {
        instance = this;
        this.config = PQCore.getConfig("crafting.yml");
        loadConfig();
        PQCore.getInstance().getCommand("pcrafting").setExecutor(new PCraftingCmd());
    ...
    public void loadConfig(){
        File file = new File(PQCore.getInstance().getDataFolder(), "crafting.yml");
        if (!file.exists()) {
            file.getParentFile().mkdirs();
            PQCore.getInstance().saveResource("crafting.yml", false);
        }
        ...
        for(String s : config.getConfigurationSection("").getKeys(false)) {
            ...
            ItemStack dItem = PQCore.deserializeItem(config.getString(s + ".ditem"));
            ItemMeta im = dItem.getItemMeta();
            im.setDisplayName(dname);
            im.setLore(dlore);
            dItem.setItemMeta(im);

            Set<ItemStack> materials = config.getStringList(s + ".materials").stream().map(PQCore::deserializeItem).collect(Collectors.toSet());
            ...
            * */
            ...
            ItemStack product = PQCore.deserializeItem(config.getString(s + ".product.item"));
